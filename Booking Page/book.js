...
const bookBtn = document.querySelectorAll('.submit-btn');
...
if (contentParam === 'container0') {
    background.style.backgroundImage = "url('../images/ferrari-2.jpg')";
    background.style.backgroundSize= 'cover';
    background.style.backgroundPosition = 'center';
  }  
else if (contentParam === 'container1') {
  ...
  const containerToShow = document.getElementById(contentParam);
    ...
    let bookBtns = document.querySelectorAll('.submit-btn'); // Assuming 'bookBtn' is the class name

    bookBtns.forEach((bookBtn) => {
        bookBtn.addEventListener('click', () => {
            console.log('Button clicked');
            alert('Your booking has been confirmed');
            window.location.href = '/Booking Page/book.html?content=' + contentParam; // Redirect with query parameter
        });
    });